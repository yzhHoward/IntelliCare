...
def set_logger(log_path: str, mode="a") -> None:
...
class Trainer:
    ...
    def __init__(
        ...
        if enable_logging:
            ...
            set_logger(self.exp_path, "w" if new_logging else "a")
        else:
            self.exp_path = None
    ...
    def train(
        ...
        for epoch in range(1, epochs + 1):
            ...
            logger.info(f"--- Epoch {epoch}, train loss: {sum(training_loss) / len(training_loss):.4f} ---")
            # if self.exp_path is not None:
            #     self.save_ckpt(os.path.join(self.exp_path, "last.ckpt"), epoch)

            # validation
            if val_dataloader is not None:
                scores = self.evaluate(val_dataloader)
                # logger.info(f"--- Eval epoch-{epoch}, step-{global_step} ---")
                # logger.info({"{}: {:.4f}".format(key, scores[key]) for key in scores.keys()})
                # for key in scores.keys():
                #     logger.info("{}: {:.4f}".format(key, scores[key]))
                logger.info(" ".join(["{}: {:.4f}".format(key, scores[key]) for key in scores.keys()]))
                # save best model
                if monitor is not None:
                    score = scores[monitor]
                    if is_best(best_score, score, monitor_criterion):
                        logger.info(
                            f"New best {monitor} score ({score:.4f}) at epoch {epoch}"
                        )
                        best_score = score
                        if self.exp_path is not None:
                            self.save_ckpt(os.path.join(self.exp_path, "best.ckpt"), epoch)
                            
            if hasattr(self.model, 'update'):
                self.model.update(epoch, train_dataloader)
            if hasattr(self.model, 'backbone') and hasattr(self.model.backbone, 'update'):
                self.model.backbone.update(epoch, train_dataloader)
    ...
    def save_ckpt(self, ckpt_path: str, epoch) -> None:
